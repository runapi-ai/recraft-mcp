<h1 align="center">Recraft MCP Server</h1>

<p align="center">
  <strong>Create Recraft AI tasks through RunAPI — 2 endpoints, 2 models — then poll status and check pricing. One MCP server, one API key.</strong>
</p>
...
`@runapi.ai/recraft-mcp` is a focused MCP server for the **Recraft** model line on RunAPI.
It lets an MCP-compatible assistant create Recraft tasks, poll their status, and check current pricing — all through a single RunAPI API key.
...
| Tool | Auth | Purpose |
|---|---|---|
| `remove_background` | Yes | Create a Recraft task (remove background) and optionally poll until it reaches a terminal status. Returns the task id, status, output URLs, and a price snapshot. |
| `upscale_image` | Yes | Create a Recraft task (upscale image) and optionally poll until it reaches a terminal status. Returns the task id, status, output URLs, and a price snapshot. |
| `get_task` | Yes | Fetch the current status and latest payload for an existing task. |
| `check_pricing` | No | Look up the current price for a Recraft model and endpoint. |

---

## Models

Recraft covers 2 models across 2 endpoints. Each tool accepts the models listed for it:
...
The assistant calls `check_pricing` to confirm the cost, then `remove_background`, and returns the task id, status, and output URLs.
