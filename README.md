...
RunAPI MCP server for the **Recraft** model line. Create tasks,
poll their status, and check pricing through a single RunAPI API key.

## Tools

- `remove_background` — create a Recraft task (remove background) and (optionally) poll until it reaches a terminal status. Returns the task id, status, output URLs, and a price snapshot. Models: `recraft-remove-background`, `recraft-crisp-upscale`.
- `upscale_image` — create a Recraft task (upscale image) and (optionally) poll until it reaches a terminal status. Returns the task id, status, output URLs, and a price snapshot. Models: `recraft-remove-background`, `recraft-crisp-upscale`.
