...
describe("recraft stdio MCP server", () => {
  ...
  it("exposes the model-line tools over the real stdio transport", async () => {
    ...
    client = new Client({ name: "recraft-mcp-test", version: "0.1.0" });
    ...
    expect(names).toEqual(["check_pricing","get_task","remove_background","upscale_image"]);
    ...
    for (const model of ["recraft-remove-background","recraft-crisp-upscale"]) {
